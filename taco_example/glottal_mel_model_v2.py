...
class BohaoDecoder(nn.Module):
    ...
    def forward(self, memory, memory_lengths):
        ...
        while True:
            ...
            print("Not finish",not_finished,dec,torch.sigmoid(gate_output),gate_output.size())
...
class TransformerModel(nn.Module):
    def __init__(self, ntoken: int, d_model: int, nhead: int, d_hid: int,
        ...
        self.ag = ag
        self.inversemel_pressure = torchaudio.transforms.InverseMelScale(n_stft=int(1+self.ag.filter_length/2), n_mels = self.ag.n_mel_channels,f_max = self.ag.mel_fmax,max_iter=1)
        self.inversemel_velocity = torchaudio.transforms.InverseMelScale(n_stft=int(1+self.ag.filter_length/2), n_mels = self.ag.n_mel_channels,f_max = self.ag.mel_fmax,max_iter=1)
    ...
    def forward(self, src):
        ...
        glottal_decoder_output, gate_outputs, alignments, mel_lengths = self.transformer_glottal_source_decoder(glottal_encoder_output,memory_lengths)
...
def train():
    ...
    for i, batch in enumerate(train_loader):
        ...
        #loss = criterion(pred_y_target,target.double())
        loss = criterion(pred_y_target, pred_y_gate_output ,target.double(),gate.double())
