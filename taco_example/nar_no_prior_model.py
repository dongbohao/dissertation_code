...
encoder_head=2
dropout=0.1
model = FastSpeech(encoder_n_layer,encoder_head,dropout).to(device)
...
if load_from_checkpoint:
    ...
    checkpoint = torch.load(r"checkpoint_v9_no_p_%s.pt"%str(sec_name),map_location=device)
...
buffer_train = get_data_to_buffer("train_%s.txt"%str(sec_name))
...
buffer_val = get_data_to_buffer("val_%s.txt"%sec_name)
dataset_val = BufferDataset(buffer_val)
...
buffer_test = get_data_to_buffer("test_%s.txt"%sec_name)
dataset_test = BufferDataset(buffer_test)
...
buffer_sample = get_data_to_buffer("samples.txt")
dataset_sample = BufferDataset(buffer_sample)

sample_loader = DataLoader(dataset_sample,
                                 batch_size=hp.batch_expand_size * hp.batch_size,
                                 shuffle=False,
                                 collate_fn=collate_fn_tensor,
                                 drop_last=True,
...
def train():
    ...
    for i, batchs in enumerate(training_loader):
        ...
        #if i >=0:
        #    break
    return list(map(lambda x:x/hp.batch_expand_size/(i+1),[history_train_loss,vv_train_loss,a_train_loss,b_train_loss]))
...
def valid():
    ...
    print("Epoch valid size", len(val_loader) * hp.batch_expand_size)
    for i, batchs in enumerate(val_loader):
        ...
        #if i >=0:
        #    break
    return list(map(lambda x:x/hp.batch_expand_size/(i+1),[history_val_loss,vv_val_loss,a_val_loss,b_val_loss]))
...
def ttst():
    ...
    print("Epoch test size", len(test_loader) * hp.batch_expand_size)
    for i, batchs in enumerate(test_loader):
        ...
        #if i >=0:
        #    break
    return list(map(lambda x:x/hp.batch_expand_size/(i+1),[history_test_loss,vv_test_loss,a_test_loss,b_test_loss]))
...
def val():
    for e in range(0,hp.epochs):
        ...
        checkpoint_path = "checkpoint_v9_no_p_%s.pt"%sec_name
        torch.save({
            'epoch': e,
...
def print_spectrogram(pred_y_mel,ground_truth = False,pic_name = ""):
    ...
    np_path = os.path.join("data","np_data", "v9_no_%s.npy"%pic_name)
    np.save(np_path,
                pred_y_mel, allow_pickle=False)
    print("Save np data",np_path)
    #log_spectro = log_spectro[:,:gate_index]

    # Plotting the short-time Fourier Transformation
    plt.figure(figsize=(20, 5))
    # Using librosa.display.specshow() to create our spectrogram
    librosa.display.specshow(log_spectro, sr=hp.sampling_rate, x_axis='time', y_axis='hz', hop_length=hp.hop_length,
                             cmap='magma', fmax=80)
    plt.colorbar(label='Decibels')

    if pic_name:
        plt.savefig("v9_no_%s.png"%pic_name)
    else:
        if ground_truth:
            plt.savefig("v9_no_groun_truth.png")
        else:
            plt.savefig("v9_no_test_predict.png")




def tt_dataset():
    for i, batchs in enumerate(sample_loader):
        for j,db in enumerate(batchs):
            character = db["text"].long().to(device)
            mel_target = db["mel_target"].float().to(device)
            duration = db["duration"].int().to(device)
